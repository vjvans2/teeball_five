class GamesController < ApplicationController
  before_action :_set_game, only: [ :show, :edit, :update, :destroy ]
  ...
  def index
    ...
    @games = Game.all.order(date: :desc, location: :asc)
  end
  ...
  def create
    ...
    @game.season_id = 1
    if @game.save
      gameday_team = _create_gameday_team(@game.id)
  ...
  def _create_gameday_team(game_id)
